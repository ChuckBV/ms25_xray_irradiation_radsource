...
p1 <- ggsurvplot(km_fit2, 
                 data = indiv_data2, 
                 pval = FALSE,
                 palette  = c("black", "black"),   # both curves in black
                 linetype = c("solid", "dotted"),   # one solid, one dotted
                 legend.title = "Dose", 
                 legend.labs = c("0 Gy","300 Gy")) 
...
ggsave(filename = "survival_curve.tif", 
       plot = p1$plot,
       device = "tiff",
...
## Examination of longevity by sex, all doses pooled
...
Final step--save both p1 and p2 in an RData file so the graphics can be tweaked and combined in a separate session.

```{r save_plots}
save(p1, p2, file = "surv_plots.RData")
```
